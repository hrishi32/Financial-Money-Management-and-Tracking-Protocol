...
<!-- <a href=<?php echo "\"account.php?id=".$row['reqId']."\""; ?> > -->
							<div class="container">
								<ul>
								<?php $sql="SELECT * FROM Request where srcAcc = '$accId'  ";
// echo $sql;
$result = mysqli_query($db,$sql);
$reqId=$_GET['id'];
	$count = 0;
	...
	if($result) {
		...
		?>
		<div style="padding: 14px 20px; margin: 8px 0;">
		<li><strong>Request ID: </strong> <?php echo $_GET['id']; ?><br/>
		...
		<li><strong>approved:</strong> <?php echo $row['approved']; ?><br/>
		 <li>Amount:<input type='integer' name='amount' style="width: 40%" value=<?php echo $row['amount']  ;?>  ></li><br>
		<input type='hidden' name='reqId' value=<?php echo $_GET['id']; ?> >
	</div>
		<input type='submit' value='Update Request' style="background-color: #4CAF50; color: white; padding: 14px 20px; margin: 8px 0; border: none;  cursor: pointer; width: 80%;">
	</form>
		<?php 
	} 
	else {
